  ...
  const alterarQuantidade = (idServico, delta, tempoDoServico) => {
    if (delta > 0) {
      ...
      if (tempoFuturo > tempoMaximoPermitido) {
        alert(`O horário das ${selectedHorario} permite no máximo ${maxSlotsDisponiveis} hora(s) consecutiva(s). Escolha um serviço mais rápido ou outro horário.`);
        return; 
      }
    }
  ...
  return (
    <div className="flex bg-gray-50 min-h-screen font-sans w-full relative">
      <aside className="fixed left-0 top-0 bottom-0 w-[70px] md:w-20 bg-white border-r border-gray-200 flex flex-col items-center pt-6 pb-6 gap-4 md:gap-6 shadow-sm z-20 overflow-x-hidden">
        <div className="w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br from-rose-400 to-rose-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg text-lg md:text-xl mb-2 md:mb-4 shrink-0">CL</div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-gray-100">
              ...
              {profissionais.length === 0 ? (
                <p className="text-sm text-gray-500">Nenhuma profissional cadastrada no sistema.</p>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {profissionais.map(prof => (
                    <div key={prof.id} onClick={() => { setSelectedProfissional(prof.id); setSelectedDate(null); }} className={`cursor-pointer p-4 rounded-xl border-2 transition flex items-center gap-3 ${selectedProfissional === prof.id ? 'border-rose-500 bg-rose-50' : 'border-gray-100 hover:border-rose-200'}`}>
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shadow-sm ${selectedProfissional === prof.id ? 'bg-rose-500 text-white' : 'bg-gray-100 text-gray-500'}`}>{prof.sigla}</div>
                      <div>
            ...
            <div className={`bg-white rounded-2xl p-5 md:p-6 shadow-sm border border-gray-100 transition-opacity duration-300 ${!selectedProfissional ? 'opacity-50 pointer-events-none grayscale' : 'opacity-100'}`}>
              ...
              <div className="grid grid-cols-7 gap-1 md:gap-2 text-center">
                ...
                {Array.from({length: 31}, (_, i) => i + 1).map(day => {
                  ...
                  return (
                    <button key={day} disabled={isFolga || isPassado} onClick={() => { setSelectedDate(day); setActiveModal('horarios'); }} className={`relative h-10 rounded-lg flex justify-center items-center text-sm font-medium transition ${isFolga || isPassado ? 'bg-red-50 text-red-400 cursor-not-allowed line-through border border-red-100' : 'hover:bg-rose-50 hover:text-rose-600 border border-transparent hover:border-rose-200 cursor-pointer text-gray-700'}`}>
                      {day}
                    </button>
                  );
                })}
                      ...
                      <div key={ag.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="flex items-center gap-4">
                          <div className="w-14 h-14 bg-rose-50 rounded-xl flex flex-col items-center justify-center border border-rose-100 shrink-0">
                            ...
                            <span className="text-rose-400 text-[10px] uppercase font-bold">Mês</span>
                          </div>
                          <div>
                        ...
                        <button onClick={() => cancelarAgendamento(ag.id)} disabled={!permissaoCancelar} className={`px-5 py-2.5 rounded-xl text-sm font-bold transition shrink-0 w-full md:w-auto ${permissaoCancelar ? 'bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 border border-red-100' : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'}`}>
      ...
      {activeModal === 'horarios' && (
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-[95%] sm:w-full max-w-sm relative p-6">
            <button onClick={() => setActiveModal(null)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-800 transition"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
            ...
            <div className="grid grid-cols-3 gap-3">
              {todosHorarios.map(hora => {
                ...
                return (
                  <button key={hora} disabled={bloqueado} onClick={() => { setSelectedHorario(hora); setActiveModal('servicos'); }} className={`py-3 rounded-xl text-sm font-bold transition border ${bloqueado ? 'bg-red-50 text-red-400 border-red-100 line-through cursor-not-allowed' : 'bg-white text-gray-600 border-gray-200 hover:border-rose-300 hover:text-rose-600 hover:bg-rose-50 shadow-sm'}`}>
                    {hora}
                  </button>
                );
              })}
      ...
      {activeModal === 'servicos' && (
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-[95%] sm:w-full max-w-md relative p-6 max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <p className="text-sm text-gray-500 mb-6">Agendamento para dia <span className="font-bold text-gray-700">{selectedDate}</span> às <span className="font-bold text-gray-700">{selectedHorario}</span>.</p>
                ...
                return (
                  <div key={servico.id} className={`flex items-center justify-between p-3 rounded-xl border transition ${qtd > 0 ? 'border-rose-300 bg-rose-50/50' : 'border-gray-100'}`}>
                    ...
                    <div className="flex items-center bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm shrink-0">
                      <button onClick={() => alterarQuantidade(servico.id, -1, servico.tempo)} className="w-8 h-8 flex items-center justify-center text-gray-500 hover:bg-rose-50 transition">-</button>
            ...
            <div className="flex flex-col-reverse sm:flex-row justify-end gap-3">
              <button onClick={resetarSelecao} className="w-full sm:w-auto px-6 py-3 text-gray-600 font-bold bg-white border border-gray-200 hover:bg-gray-50 rounded-xl transition shadow-sm">Cancelar Tudo</button>
              <button onClick={handleConfirmarAgendamento} className="w-full sm:w-auto px-6 py-3 bg-green-500 text-white font-bold rounded-xl hover:bg-green-600 transition shadow-md flex items-center justify-center gap-2">Confirmar</button>
            </div>
          </div>
        </div>
      )}
